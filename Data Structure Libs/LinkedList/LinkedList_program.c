...
ERROR_STATUS List_xInsertNodeNth(Node **Copy_ppHead , ListType Copy_xData , uint32 Copy_u32Position)
{
...
ERROR_STATUS List_xModifyNodeNth(Node **Copy_ppHead , ListType Copy_xData , uint32 Copy_u32Position)
{
   if(LIST_IS_EMPTY == List_xIsEmpty(*Copy_ppHead))
    {
	...
	CurrnetNode->Data = Copy_xData;

	return E_OK;
}

ListType List_xDeleteFirstNode(Node **Copy_ppHead )
{
	if(LIST_IS_EMPTY == List_xIsEmpty(*Copy_ppHead))
    {
       // return LIST_ACCESS_EMPTYLIST_ERROR;
    }
	ListType HeadData = (*Copy_ppHead)->Data;
	Node *CurrentNode = *Copy_ppHead;
...
ERROR_STATUS List_xDeleteNode(Node **Copy_ppHead ,uint32 Copy_u32Position)
{
    if(LIST_IS_EMPTY == List_xIsEmpty(*Copy_ppHead))
    {
        return LIST_ACCESS_EMPTYLIST_ERROR;
...
uint32 List_u32Count(Node * Copy_pHead)
{
	uint32 Count = 0;
